...
var SalesforceDefaultCliUser = class _SalesforceDefaultCliUser {
  ...
  constructor(authenticator) {
    this.Ready = new Promise(async (makeReady) => {
      ...
      this.browser = await import_test.chromium.launch({ headless: true });
...
${result.exceptionStackTrace}`);
  ...
  async validateRecordLayoutsFor(recordId, page, options) {
    try {
      ...
      (0, import_test2.expect)(JSON.stringify(await orgLayouts, null, 3)).toMatchSnapshot();
    } catch (error) {
...
var SalesforceObject = class {
      ...
      validateComponentsFor: async (recordId) => {
        ...
        try {
          if (testInfo.config.updateSnapshots !== "none") {
            ...
            console.debug(`snapshot capture is on in '${testInfo.config.updateSnapshots}' mode- using implicit wait for ${safePeriod / 1e3}s to record`);
            await flexipage.ui.waitForTimeout(safePeriod);
          }
          await (0, import_test3.expect)(async () => {
            parsedComponents = await flexipage.getComponents();
            (0, import_test3.expect)(parsedComponents, "components validation").toMatchSnapshot();
  ...
  async attachPageSnapshot(page) {
    try {
      await (0, import_test3.expect)(page).toHaveScreenshot({ maxDiffPixels: 0, fullPage: true });
    } catch (ignore) {
    }
  }
