...
import { AirlineUser } from "../users/standard/User";

test.use({video: 'off'});
test.describe.parallel('@api @fcrm @airline layout validations', async () => {
    let actor: AirlineUser;

    test.beforeEach(async ({page}) => {
        await (actor = new AirlineUser(page)).Ready;
    });
