...
import { AirportUser } from "../users/standard/User";

test.use({video: 'off'});
test.describe.parallel('@api @fcrm @airport layout validations', async () => {
    let actor: AirportUser;

    test.beforeEach(async ({page}) => {
        await (actor = new AirportUser(page)).Ready;
    });
