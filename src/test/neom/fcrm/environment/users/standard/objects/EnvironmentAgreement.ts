...
export class EnvironmentAgreement extends Sobject implements IsCreatableViaApi{
    ...
    public async createViaApi(accountId?: string, recordTypeId?: string, type?: string, status?: string): Promise<string>{
        const data = {
            ...
            Account__c: accountId ? accountId : await (this.user as EnvironmentUser).account.createViaApi(),
            RecordTypeId: recordTypeId ? recordTypeId : (await this.user.api.query("select id from recordtype where name = 'Environment Agreement'") as QueryResult<any>).records[0].Id
        }
