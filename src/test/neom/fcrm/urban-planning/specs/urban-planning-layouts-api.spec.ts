...
import { UrbanPlanningUser } from "../users/standard/User";

test.use({video: 'off'});
test.describe.parallel('@api @fcrm @urban_planning layout validations', async () => {
    let actor: UrbanPlanningUser;

    test.beforeEach(async ({page}) => {
        await (actor = new UrbanPlanningUser(page)).Ready;
    });
