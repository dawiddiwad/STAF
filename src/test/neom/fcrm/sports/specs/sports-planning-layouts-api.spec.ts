...
import { SportsUser } from "../users/standard/User";

test.use({video: 'off'});
test.describe.parallel('@api @fcrm @sports layout validations', async () => {
    let actor: SportsUser;

    test.beforeEach(async ({page}) => {
        await (actor = new SportsUser(page)).Ready;
    });
