...
export class BigQuery {
    public static async importAfternoonLeadsAs(user: User): Promise<ExecuteAnonymousResult> {
        return user.api.executeApex(`
            // String query = 'SELECT String(timestamp) AS timestamp, form_id, IF((SELECT value FROM UNNEST(data) WHERE key="contact-permissions") = \\'yes\\' , false, true) as doNotCall, IF((SELECT value FROM UNNEST(data) WHERE key="contact-permissions") = \\'yes\\' , false, true) as emailOptOut, data FROM form_data.raw_data_development WHERE form_id = \\'oxagon_form\\' AND DATE(timestamp) = CURRENT_DATE()  ORDER BY timestamp';
            // BigQueryCallouts.synchronizeLeads(query);
    ...
    public static async findLeadWith(data: any, user: User) {
        ...
        return expect.poll(async () => {
            try {
                ...
                data.Id = (queryResult.records[0] as any).Id
