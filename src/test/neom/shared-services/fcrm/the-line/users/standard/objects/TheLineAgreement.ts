...
export class TheLineAgreement extends Sobject implements IsCreatableViaApi{
    ...
    public async createViaApi(accountId?: string, recordTypeId?: string, type?: string, status?: string): Promise<string>{
        const data = {
            ...
            Account__c: accountId ? accountId : await (this.user as TheLineUser).account.createViaApi(),
            RecordTypeId: recordTypeId ? recordTypeId : (await this.user.api.query("select id from recordtype where name = 'The Line Agreement'") as QueryResult<any>).records[0].Id
        }
