...
import { TheLineUser } from "../users/standard/User";

test.use({video: 'off'});
test.describe.parallel('@api @fcrm @the_line layout validations', async () => {
    let actor: TheLineUser;

    test.beforeEach(async ({page}) => {
        await (actor = new TheLineUser(page)).Ready;
    });
