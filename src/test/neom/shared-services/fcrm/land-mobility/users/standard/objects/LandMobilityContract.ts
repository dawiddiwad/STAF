...
export class LandMobilityContract extends Sobject implements IsCreatableViaApi{
    constructor(user: LandMobilityUser){
        super(user);
    }

    public async createViaApi(): Promise<string>{
        const recordTypeId = (await this.user.api.query("select id from recordtype where name = 'Land Mobility Contract'") as QueryResult<any>).records[0].Id;
        const data = {
            Status: 'Draft',
            Type__c: 'MOU',
            ContractTerm: '12',
            AccountId: await (this.user as LandMobilityUser).account.createViaApi(),
