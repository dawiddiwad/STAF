...
import { NatureReserveUser } from "../users/standard/User";

test.use({video: 'off'});
test.describe.parallel('@api @fcrm @nature_reserve layout validations', async () => {
    let actor: NatureReserveUser;

    test.beforeEach(async ({page}) => {
        await (actor = new NatureReserveUser(page)).Ready;
    });
