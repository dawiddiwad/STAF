...
export class AirlineAgreement extends Sobject implements IsCreatableViaApi{
    ...
    public async createViaApi(accountId?: string, recordTypeId?: string, type?: string, status?: string): Promise<string>{
        const data = {
            ...
            Account__c: accountId ? accountId : await (this.user as AirlineUser).account.createViaApi(),
            RecordTypeId: recordTypeId ? recordTypeId : (await this.user.api.query("select id from recordtype where name = 'NEOM Airline Agreement'") as QueryResult<any>).records[0].Id
        }
